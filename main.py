...
def main():
    ...
    if args.arch == 'resnet18':
        ...
        model = wide_resnet50_2(weights='IMAGENET1K_V1', progress=True)
    ...
    for class_name in mvtec.CLASS_NAMES:
        ...
        if not os.path.exists(train_feature_filepath):
            ...
            with open(train_feature_filepath, 'wb') as f:
                ...
                pickle.dump(train_outputs, f, protocol=4) #オーバーフロー対策
        else:
        ...
        coeff_min = 0.01
        coeff_max = 50.01
        scores = (score_map - coeff_min) / (coeff_max - coeff_min)
        ...
        plot_fig(test_imgs, scores, threshold, save_dir, class_name, ng_images, fullpath, result_dict, area_dict)
    ...
    with open(args.save_path + '/' + 'sorted_results_score.csv', 'w', newline='') as csvfile:
        fieldnames = ['ALL FULLPATH', 'SCORE']
        csvwriter = csv.DictWriter(csvfile, fieldnames=fieldnames)
        csvwriter.writeheader()
    ...
    # ソートされた結果をCSVファイルに書き込む
    sorted_results = dict(sorted(area_dict.items(), key=lambda item: item[1], reverse=True))
    with open(args.save_path + '/' + 'sorted_results_area.csv', 'w', newline='') as csvfile:
        fieldnames = ['ALL FULLPATH', 'AREA']
        csvwriter = csv.DictWriter(csvfile, fieldnames=fieldnames)
        csvwriter.writeheader()
        
        # データを書き込む
        for key, value in sorted_results.items():
            csvwriter.writerow({'ALL FULLPATH': key, 'AREA': value})


def plot_fig(test_img, scores, threshold, save_dir, class_name, ng_images, fullpath, result_dict, area_dict):
    num = len(scores)
    vmax = scores.max() * 255.
    vmin = scores.min() * 255.
    for i in range(num):
        ...
        np.savetxt(os.path.join(save_dir,'{}.csv'.format(os.path.splitext(os.path.basename(fullpath[i]))[0])), mask, delimiter=",")
        
        result_dict[fullpath[i]] = mask.max()
        
        area = np.count_nonzero(mask > threshold)
        area_dict[fullpath[i]] = area
        
        # print(os.path.splitext(os.path.basename(fullpath[i]))[0], np.count_nonzero(mask.max() > threshold))
        ...
        child = os.path.basename(os.path.split(fullpath[i])[0])
        filename_noext = os.path.splitext(os.path.basename(fullpath[i]))[0]
        
        # fig_img.savefig(os.path.join(save_dir, child + "_" + filename_noext + '_{}'.format(i)), dpi=100)
        fig_img.savefig(os.path.join(save_dir, "{}".format(int(result_dict[fullpath[i]] * 10000)) + "_" + child + "_" + filename_noext), dpi=100)
        plt.close()
